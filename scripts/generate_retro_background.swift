...
// Draw connected wireframe nodes
var nodes: [(CGFloat, CGFloat)] = []
for _ in 0..<30 {
    let x = random(width * 0.55, width - 50)
...
// Draw glow effect around wireframe area
let gradient = NSGradient(colors: [
    accentColor.withAlphaComponent(0.1),
    accentColor.withAlphaComponent(0.0)
])
let glowPath = NSBezierPath(ovalIn: NSRect(x: width * 0.5, y: -height * 0.3, width: width, height: height * 1.6))
...
// App is at x=260 (130*2), Applications at x=1060 (530*2), y=370 (185*2)
let arrowY: CGFloat = 370
let arrowStartX: CGFloat = 340
let arrowEndX: CGFloat = 980
